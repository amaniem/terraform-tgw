provider "aws" {
  region = "us-east-1"
  access_key    = "AKIA6LEUQH2WN5BSUAH4"
  secret_key = "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"
}
...
module "sg-tgw" {
  ...
  ingress-rule-1-cidrs = ["192.168.0.2/32"]
  ingress-rule-1-description = "Deloitte tgw"
...
module "transit-gateway" {
  ...
  transit_gateway_name                                                   = "deloittetgw.ca-transit-gateway"
  ...
  vpc_id                                                                 =  module.vpc.vpc-id
  aws_ec2_transit_gateway_vpc_attachment_name                            = "deloittetgw.ca-tgw-vpc-dev-attahments"
